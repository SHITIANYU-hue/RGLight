...
class Lane():
    ...
    #leaving_inb_comms_receivers = collections.OrderedDict()
    #leaving_outb_comms_receivers = collections.OrderedDict()
    ...
    #def get_rewards(self, reward_vector):
        #self.reward += np.matmul(self.discount_vector,reward_vector)

        #2)functions "update_lane_state" and "update_lane_reward" (every time a vehicle is on this lane, it will contribute to this lane's immediate state and reward)
    def reset(self):
    ...
    def update_lane_state_and_reward(self,veh_speed, veh_max_speed, position, veh_state = False, ignored = False):
        ...
        # SELF.STATE [0] = n_vehicles [1] = avg_speed
        #reachable_speed = min(veh_max_speed,self.max_speed)
        #self.delay += ( (reachable_speed - veh_speed) / reachable_speed) **2
        #if not ignored:  
        #print('position', position, 'length', self.length, 'pos_thre', Lane.position_threshold, 'pos_thre *  length', self.length * Lane.position_threshold)
        veh_queue = 0
        delay = 0
