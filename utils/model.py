...
class cos_embedding(nn.Module):
    def __init__(self,batch_size,embedding_dim,layer_size=256,n_cos=64,num_tau=8,device='cpu'):
        super(cos_embedding, self).__init__() 
        self.device=device
        ...
        self.head = nn.Linear(self.embedding_dim, self.layer_size).to(device) # cound be a cnn 
        # print('input shape 0',input_shape[0])
        self.cos_embedding = nn.Linear(self.n_cos, self.layer_size).to(device) ##to device
        self.ff_1 = nn.Linear(self.layer_size, self.layer_size).to(device) ###to device
        self.ff_2 = nn.Linear(self.layer_size, self.embedding_dim).to(device)
    ...
    def forward(self,input,num_tau=8,layer_size = 256):
        """
        ...
        cos, taus = self.calc_cos(self.batch_size,num_tau) # cos shape (batch, num_tau, layer_size)
        cos = cos.view(self.batch_size*self.num_tau, self.n_cos)
        cos_x = torch.relu(self.cos_embedding(cos)).view(self.batch_size, self.num_tau, self.layer_size) # (batch, n_tau, layer)
        x = (x.unsqueeze(1)*cos_x).view(self.batch_size*self.num_tau, self.layer_size)
        # x has shape (batch, layer_size) for multiplication –> reshape to (batch, 1, layer)
        x=torch.relu(self.ff_1(x))
        out=self.ff_2(x)
        return out.view(self.batch_size,self.num_tau,self.embedding_dim)
...
class Convolutional_Message_Passing_Framework(nn.Module):
    def __init__(self, policy, noisy, num_attention_heads, bias_before_aggregation , nonlinearity_before_aggregation, share_initial_params_between_actions, multidimensional_attention, state_first_dim_only, gaussian_mixture, n_gaussians, value_model_based, use_attention,  separate_actor_critic, n_actions, rl_learner_type, std_attention, state_vars, n_convolutional_layers, num_nodes_types, nodes_types_num_bases, node_state_dim, node_embedding_dim, num_rels, n_hidden_message, n_hidden_aggregation, n_hidden_prediction, hidden_layers_size, prediction_size, num_propagations = 1, rel_num_bases = -1, norm = True, bias = True, activation = F.relu, use_message_module = True, use_aggregation_module = False, dropout = False, resnet = False, dueling = False):
        ...
        self.cos_embedding = cos_embedding(batch_size=159,embedding_dim=32)
    ...
    def forward_d(self, graph, device, learning = False, joint = False, testing = False, actions_sizes = None):
        ...
        if 'actor' in self.rl_learner_type.lower() and 'critic' in self.rl_learner_type.lower() :
            if self.separate_actor_critic :  
                ...
                subgraph.ndata['hid']=self.cos_embedding(batch_size=subgraph.nadata['hid'].size()[0]).forward(subgraph.ndata['hid']) ##
                
                if self.policy != 'binary':
                    nt, hid, v = self.prediction_module_d.predict(subgraph,device, testing)             
                else:
                    hid, a, v = self.prediction_module_d.predict(subgraph,device, testing)  
                    # print('hid',hid)
                ...
                #print('hid shape',subgraph.ndata['hid'].size()[0])
                self.cos_embedding = cos_embedding(batch_size=subgraph.ndata['hid'].size()[0],embedding_dim=32)
                subgraph.ndata['hid']=self.cos_embedding.forward(subgraph.ndata['hid']) 
                #print('hid data size', subgraph.ndata['hid'].size())
                #print('nodetype data size',subgraph.ndata['node_type'].size())
                #print('nodetype data type',type(subgraph.ndata['node_type']))
                #subgraph.ndata['node_type']=torch.zeros(subgraph.ndata['node_type'].size()[0],8)
                subgraph.ndata['node_type']=subgraph.ndata['node_type'].unsqueeze(1)
                subgraph.ndata['node_type']=subgraph.ndata['node_type'].repeat(1,8)
                #print('nodetype data size',subgraph.ndata['node_type'].size())
                #print('hid data', subgraph.ndata['hid'])
                #print('node type data size:',subgraph.ndata['node_type'].size())
                #subgraph.ndata['node_type']=subgraph.ndata['node_type'].long()
                #print('hid data  type',subgraph.ndata['hid'].type())
                #subgraph.ndata['node_type']=self.cos_embedding.forward(subgraph.ndata['node_type'])
                #print('node type data',subgraph.ndata['node_type'])
                if self.policy != 'binary':
                    nt, hid, v = self.prediction_module_d.predict(subgraph, device, testing)    
                else:
                    #print("b")
            ...
            if self.dueling:
                if self.policy != 'binary':
                    ...
                    batch_size, embedding_dim=hid.shape[0],hid.shape[1]
                    #print('v shape',v.size())
                    #print('a shape',a.size())
                    Q=(v.unsqueeze(2) + (a - torch.mean(a, dim = 2).unsqueeze(2))).squeeze()
                    #print('hid size',hid.size())
                    #Q_2=torch.nn.Linear(embedding_dim,2)
                    #print('Q2 size',Q_2)
                    #print('Q value size',Q.size())

                    return hid, Q ##q value
            ...
            subgraph.copy_from_parent()    
            if self.policy != 'binary':            
                nt, hid, Q = self.prediction_module_d.predict(subgraph, device, testing)
                ...
                hid, Q = self.prediction_module_d.predict(subgraph, device, testing)
                return hid, Q 
...
class Prediction_Module_d(nn.Module):
    ...
    def __init__(self, dueling, policy, noisy, share_initial_params_between_actions, gaussian_mixture, n_gaussians, value_model_based, n_actions, rl_learner_type, use_message_module, num_nodes_types, num_bases, n_hidden, hidden_layers_size, in_feat , out_feat , bias = True, activation = F.relu, norm = True, dropout = False):
        super(Prediction_Module_d, self).__init__() 
        ...
        def apply_func(nodes):
            ...
            if self.n_hidden == 0:
                ...
                if 'critic' in self.rl_learner_type.lower(): 
                    ...
                    if self.use_message_module:
                        #print('node data shape',nodes.data['hid'].unsqueeze(2).size())
                        #print('critic shape',critic_w_prediction_input.size())
                        hid_data=nodes.data['hid'].unsqueeze(2).reshape(nodes.data['hid'].unsqueeze(2).size()[0]*nodes.data['hid'].unsqueeze(2).size()[1],nodes.data['hid'].unsqueeze(2).size()[2],nodes.data['hid'].unsqueeze(2).size()[3])
                        critic_pred=critic_w_prediction_input.reshape(critic_w_prediction_input.size()[0]*critic_w_prediction_input.size()[1],critic_w_prediction_input.size()[2],critic_w_prediction_input.size()[3])
                        #value = torch.bmm(nodes.data['hid'].unsqueeze(2), critic_w_prediction_input).squeeze()
                        value = torch.bmm(hid_data,critic_pred).squeeze().to(device)
                        #value=value.reshape(critic_w_prediction_input.size()[0],critic_w_prediction_input.size()[1])
                        #print('value size',value.size())
                    ...
                    if self.bias:
                        #print('bias size',critic_bias_prediction_input.size())
                        critic_bias_prediction_input=critic_bias_prediction_input.reshape(critic_bias_prediction_input.size()[0]*critic_bias_prediction_input.size()[1],1)
                        value = value + critic_bias_prediction_input

                    value = value.squeeze()
                    value=value.reshape(nodes.data['hid'].size()[0],nodes.data['hid'].size()[1])
                ...
                if 'actor' in self.rl_learner_type.lower(): 
                    ...
                    if self.use_message_module:
                        hid_data2=nodes.data['hid'].unsqueeze(2).reshape(nodes.data['hid'].unsqueeze(2).size()[0]*nodes.data['hid'].unsqueeze(2).size()[1],nodes.data['hid'].unsqueeze(2).size()[2],nodes.data['hid'].unsqueeze(2).size()[3])
                        actor_pred=actor_w_prediction_input.reshape(actor_w_prediction_input.size()[0]*actor_w_prediction_input.size()[1],actor_w_prediction_input.size()[2],actor_w_prediction_input.size()[3])
                        actions = torch.bmm(hid_data2, actor_pred).squeeze()
                    ...
                    if self.bias:
                        #print('actor shape',actor_bias_prediction_input.size())
                        actor_bias_prediction_input=actor_bias_prediction_input.reshape(actor_bias_prediction_input.size()[0]*actor_bias_prediction_input.size()[1],2)
                        actions = actions + actor_bias_prediction_input

                    actions = actions.squeeze()       
                    actions= actions.reshape(nodes.data['hid'].size()[0],nodes.data['hid'].size()[1],2)
