...
from utils.recent_utils.system_functions import *
from utils.recent_utils.viz_functions import *
...
#         np.random.seed(seed)
        ...
        #self.env.r = np.random.RandomState(0)   
        #if seed == 0 :#or seed ==1:
            #print("random", random.random())
            #print("np",self.env.r.rand(2,3))
            #print(torch.rand(2,3))
        # SETUP CLASSES , VARIABLES AND NETWORKS THAT ARE NEEDED TO RUN THE EXPERIMENT
        self.setup(seed, n_workers)  
        
    def run(self, trips_dict, epoch, seed, num, n_workers, memory_queue,  request_end, learn_model_queue, comput_model_queue, comput_model2_queue,reward_queue, baseline_reward_queue, Policy_Type, mode, greedy_reward_queue = None, tested_end = None, tested = None, convert_to_csv=False):
        ...
        #self.env.steps_dict = collections.OrderedDict()
        ...
        self.env.comput_model2_queue = comput_model2_queue
        ...
        # DRAW GRAPH OF INTEREST (USED BY THE ML MODEL) AND SAVE IT 
        ...
        # GENERATE ML MODEL AND SEND IT TO THE COMPUT AND LEARN PROCESSES
        #print('STEP A ')
        if epoch == 0:
            #print('STEP B ')
            #print('self.env.seed', self.env.seed, 'self.env.n_workers', self.env.n_workers, 'hyp' , self.env.seed % self.env.n_workers ==0)
            #print('self.env.n_workers', self.env.n_workers)
            #print('hyp' , self.env.seed % self.env.n_workers ==0)
            if self.env.seed % self.env.n_workers ==0:
                ...
                comput_model2_queue.put(self.env.env_params) # SEND THE ENV/MODEL TO THE COMPUT
                #print("MODEL SENT")
            ...
            if mode == 'train' or not self.env.env_params.additional_params['sequential_computation']:
                if self.env.seed % self.env.n_workers ==0:
                    ...
                    comput_model2_queue.put(self.env.model2)
        ...
        if self.env.seed % self.env.n_workers ==0:
            ...
            #time.sleep(30)
        """
        # INFINITE STEPS TO CREATE EXPERIENCES WHICH ARE PERIODICALLY SENT TO THE COMMON LEARNER AND APPENDED TO MEMORY BUFFER(S)
        for s in range(self.env.env_params.additional_params['nb_steps_per_exp']):
            self.step()
            if self.env.env_params.additional_params['mode'] == 'train' and self.env.seed == 0 and ( self.env.step_counter +1 ) % self.env.env_params.additional_params['nb_steps_per_exp'] == 0:
                clear_output(wait=True)   
            if self.env.env_params.additional_params['mode'] == 'test' and self.env.step_counter % self.env.env_params.additional_params['n_avg_test'] == 0:
                self.send_results()
            ...
            while self.env.trips_to_complete:
                ...
                if False:
                    if self.env.step_counter >= int(self.env.env_params.additional_params['nb_steps_per_exp']):
                        break
                if self.env.step_counter % 8000 ==0:
    ...
    def setup(self,seed, n_workers):
        ...
        #if self.env.env_params.additional_params['mode']=='train' or (self.env.env_params.additional_params['mode']=='test' and not self.env.env_params.additional_params['real_net']) : 
        ...
        #if self.env.seed == 0:
            #print("entering_edges", self.env.entering_edges)
            #print("leaving_edges", self.env.leaving_edges)
    ...
    def step(self):# INITIALIZE A NEW STEP 
        ...
        if self.env.seed % self.env.n_workers ==0:
            s = time.time()
        # RESET  
        #self.env.step_dic_name = 'step_'+str(self.env.step_counter)
        #self.env.steps_dict[self.env.step_dic_name] = collections.OrderedDict()
        self.reset_agents()
        self.reset_lanes()
        self.reset_metrics()
        if self.env.env_params.additional_params['print_time_gating']:
        ...
        #for idx,tl_id in enumerate(self.env.Agents):
            #print('tl_id : ', tl_id, 'idx : ', idx)
            #if self.env.seed == 0 : 
                #if idx == 0:
                    #print('phase bef :', self.env.traci_connection.trafficlight.getSubscriptionResults(tl_id)[traci.constants.TL_CURRENT_PHASE])
        _ = self.env.step(self.rl_actions())
        #if self.env.seed == 0 : 
            #for idx,tl_id in enumerate(self.env.Agents):
                #if idx == 0:
                    #print('action :', self.env.Actions[tl_id])
                    #print('phase aft : ', self.env.traci_connection.trafficlight.getSubscriptionResults(tl_id)[traci.constants.TL_CURRENT_PHASE])
        if self.env.env_params.additional_params['print_time_gating']:
            if self.env.seed % self.env.n_workers == 0  :
        ...
        #SEND A BATCH TO TRAINER/EVALUATOR IF TRAIN-Q-Learning and time constraints
...
##########################################################################################################################################      
    ...
    def reset_agents(self):
        ...
        for tl_id in self.env.Agents:
            ...
            if self.env.env_params.additional_params["graph_of_interest"] == "tl_connection_lane_graph" or self.env.env_params.additional_params["graph_of_interest"] == 'full_graph':
                ...
                #print("self.env.Agents[tl_id].phases_defs[-1]",self.env.Agents[tl_id].phases_defs[-1], "self.env.Agents[tl_id].current_phase",self.env.Agents[tl_id].current_phase)
                while self.env.Agents[tl_id].phases_defs[-1].state != self.env.Agents[tl_id].current_phase:
                    #print('phases_defs', self.env.Agents[tl_id].phases_defs)
    ...
    def gen_vehicles(self):
        #if self.env.seed == 1 :#or self.env.seed ==1:
            #print("random", random.random())
            #print("np",self.env.r.rand(2,3))
            #print(torch.rand(2,3))
        if (self.env.step_counter % (self.env.env_params.additional_params["demand_duration"])) == 0 or self.env.step_counter == 1:
        ...
        for idx,i in enumerate(range(self.env.env_params.additional_params["N_VEH_SAMPLES"])):
            ...
            #if self.env.seed == 0 and idx == 0:
                #print("step counter", self.env.step_counter, "sample", sample)
            #sample2 = random.random()
            #if idx == 0 and self.env.seed ==1 :
                #print("self.env.step", self.env.step_counter)
                #print("idx:", i,"sample", sample)
                #print("idx:", i, "sample2", sample2)
            if sample <= (self.env.env_params.additional_params["PROB_VEH"] *(1+self.env.demand_adjuster)):
                ...
                while True:
                    try:
                        ...
                        #print("veh_id", entering_edge, leaving_edge)
                        break
                ...
                """
                    ...
                    if self.env.env_params.additional_params["grid"]:
                        if set([i for i in entering_edge]) != set([i for i in leaving_edge]):
                        #if entering_edge[entering_edge.find('/')-1:entering_edge.find('/')+2] != leaving_edge[leaving_edge.find('/')-1:leaving_edge.find('/')+2]:
                            valid = True
                    else:
                        if entering_edge.split('-')[-1] != leaving_edge.split('-')[-1]:
                        #if entering_edge != str('-' + leaving_edge) and leaving_edge != str('-' + entering_edge):
                            valid = True
    ...
    def update_gen_probs(self):
        #if self.env.seed == 0 :#or self.env.seed ==1:
            #print("random", random.random())
            #print("np",self.env.r.rand(2,3))
            #print(torch.rand(2,3))

        self.env.demand_adjuster = np.absolute(random.gauss(0, self.env.env_params.additional_params["demand_variance"]))
        #self.env.demand_adjuster = np.absolute(self.env.r.normal(loc=0, scale=self.env.env_params.additional_params["demand_variance"], size=None))
        self.env.entering_adjuster = np.absolute([random.gauss(0, self.env.env_params.additional_params["lane_demand_variance"]) for i in range(len(self.env.entering_edges))])        
        #self.env.entering_adjuster = np.absolute(self.env.r.normal(loc=0, scale = self.env.env_params.additional_params["lane_demand_variance"], size=len(self.env.entering_edges)))
        self.env.leaving_adjuster = np.absolute([random.gauss(0, self.env.env_params.additional_params["lane_demand_variance"]) for i in range(len(self.env.leaving_edges))])
        #self.env.leaving_adjuster = np.absolute(self.env.r.normal(loc=0, scale = self.env.env_params.additional_params["lane_demand_variance"], size=len(self.env.leaving_edges)))
        #print("entering_edges_probs", self.env.entering_edges_probs)
        #print("entering_adjuste", self.env.entering_adjuster)
        self.env.new_entering_edges_probs =  self.env.entering_edges_probs * (1+self.env.entering_adjuster) 
        self.env.new_entering_edges_probs = self.env.new_entering_edges_probs -  self.env.new_entering_edges_probs.max()
        self.env.new_leaving_edges_probs = self.env.leaving_edges_probs * (1+self.env.leaving_adjuster)
        self.env.new_leaving_edges_probs = self.env.new_leaving_edges_probs - self.env.new_leaving_edges_probs.max()
        ...
        #if self.env.seed == 0 :        
            #print("demand adjuster", self.env.demand_adjuster)
            #print("entering adjuster", self.env.entering_adjuster)
            #print("leaving adjuster", self.env.leaving_adjuster)
            #print("new entering edges probs", self.env.new_entering_edges_probs)
            #print("new leaving edges probs", self.env.new_leaving_edges_probs)
    ...
    def add_vehicles(self):
        ...
        if self.env.env_params.additional_params['gen_trips_before_exp']:
            #for veh_id in self.env.traci_connection.FSubscriptionResults[VAR_DEPARTED_VEHICLES_IDS]:
            for veh_id in traci.simulation.getDepartedIDList():
                if self.env.traci_connection.vehicle.isRouteValid(veh_id):    
                    #if self.env.seed == 0:
                       #print("veh_id",veh_id, self.env.trips_dict[veh_id])
        ...
        #for veh_id in self.env.traci_connection.simulation.getSubscriptionResults[traci.constants.LAST_STEP_VEHICLE_ID_LIST]:
                    ...
                    if graph_name == self.env.env_params.additional_params['graph_of_interest']:
                        ...
                        if "lane" in graph_name:
                            ...
                            if count>0:
                                ...
                                #print("bef",graph.edata['rel_type'].size())
                                graph.add_edges(src,dst) #, {'rel_type':edge_type, 'norm':edge_norm})
                                #print("aft",graph.edata['rel_type'].size())
                                #print(edge_type.size())
                                #print(torch.cat((graph.edata['rel_type'][:-len(src)],torch.LongTensor(edge_type).squeeze()),0).squeeze().size())
    ...
    def update_veh_state_and_reward(self):
        ...
        for veh_id in self.env.traci_connection.vehicle.getIDList():
            ...
            if self.env.env_params.additional_params['mode'] == 'test' or self.env.env_params.additional_params['save_extended_training_stats']:
                ...
                self.env.co2 += co2
            #self.env.traci_connection.vehicle.getCO2Emission(veh_id)
    ...
    def update_state_DNN(self):
        ...
        for tl_id in self.env.Agents:
            ...
            #if self.env.seed == 0:
            #print("tl_id",tl_id)
            #print("n_phases", self.env.Agents[tl_id].n_phases)
            ...
            if 'active' in self.env.env_params.additional_params['phase_vars']:
                ...
                for idx, phase in enumerate(self.env.Agents[tl_id].orig_phases_defs):
                    #print("phase",phase.state)
                    #print("current_phase", self.env.Agents[tl_id].current_phase)
            ...
            for lane_id in self.env.Agents[tl_id].inb_lanes + self.env.Agents[tl_id].outb_lanes:
                    ...
                    #print("lane state",self.env.Lanes[lane_id].state + [self.env.Lanes[lane_id].std_length] if self.env.env_params.additional_params['std_lengths'] else [self.env.Lanes[lane_id].length] )
                if self.env.env_params.additional_params['std_speeds']:
                    self.env.Lanes[lane_id].state[1] /= self.env.Lanes[lane_id].max_speed
                state.extend(self.env.Lanes[lane_id].state)# + [self.env.Lanes[lane_id].std_length] if self.env.env_params.additional_params['std_lengths'] else [self.env.Lanes[lane_id].length])
                ...
                if graph_name == self.env.env_params.additional_params['graph_of_interest']:
                    ...
                    if graph_name == 'full_graph':
                        ...
                        if 'next_yellow' in self.env.env_params.additional_params['phase_vars']:
                            #if self.env.seed==0 and tl_idx == 0:   
                                #print("tl_id", tl_id)
                                #print("PHASE IDX",self.env.Agents[tl_id].current_phase_idx )
                                #print("PHASE",self.env.Agents[tl_id].current_phase )
                                #print("Phase Defs", self.env.Agents[tl_id].phases_defs)
                                #print("Phase Defs", self.env.Agents[tl_id].orig_phases_defs)
                                #print("Phase Defs[0]", self.env.Agents[tl_id].phases_defs[0])
                            for idx,phase in enumerate(self.env.Agents[tl_id].phases_defs):
                                #print("p_phase", phase)
                                #phase_name = str(tl_id+"_phase_"+str(idx))                                
                                phase_name = str(tl_id+"_phase_"+str(phase.idx))
                                #print('phases',phase.state.lower())
    ...
    def send_experience_batch(self):
        ...
        self.env.all_graphs = []
            ...
            #try:  
                ...
                #if self.env.env_params.additional_params['Policy_Type'] == "Q_Learning":
                    #self.env.memory_queue.send(self.env.all_graphs[1:-1])
                #if self.env.env_params.additional_params['save_extended_training_stats']:
                    #self.env.reward_queue.send((float(self.env.global_reward),float(self.env.global_delay),float(self.env.global_queues), float(self.env.global_co2)))
                #else:
    ...
    def rl_actions(self):
        #print("self.env.tested", self.env.tested, "greedy", self.env.greedy, "baseline", self.env.baseline)
        activated_tls = []
        ...
        if self.env.seed % self.env.n_workers == 0 :
            ...
            #for lane_id in ['420905129#0_0', '420905129#1_0']:
                #print("lane " + lane_id)
                #print("state :", self.env.Lanes[lane_id].state)
                #if 'strong' in self.env.tested :
                    #print("nb_stop_veh :", self.env.Lanes[lane_id].nb_stop_veh)
                    #print("nb_mov_veh :", self.env.Lanes[lane_id].nb_mov_veh)
        ...
        # ACTOR-CRITIC
            ...
            self.env.request_end.send((self.env.current_graphs[self.env.graph_of_interest] if self.env.env_params.additional_params['GCN'] else self.env.state, self.env.original_graphs["tl_graph"].number_of_nodes(), torch.LongTensor(action_size).view(-1), self.env.Targets)  if not self.env.baseline else 'N/A')
            #elif not self.env.env_params.additional_params['GCN']:
                #self.env.request_end.send((self.env.state,len(self.env.Agents), torch.LongTensor(action_size).view(-1)) if not self.env.baseline else 'N/A')
        ...
        if not self.env.baseline :
            ...
            actions = list(actions)
                ...
                #for idx,tl_id in enumerate(self.env.Agents):
                    #if idx == 0:
                        #print('orig action :', actions[idx])
            #print("tested", self.env.tested, "actions", actions)
            ...
            if not self.env.greedy and not self.env.env_params.additional_params['gaussian_mixture'] and not self.env.env_params.additional_params['noisy']:
                ...
                for idx,tl_id in enumerate(self.env.Agents):
                    ...
                    if sample < self.env.eps_threshold:
                        #if idx == 0:
                            #print("D", 'self.env.eps_threshold', self.env.eps_threshold)
                        actions[idx] = torch.from_numpy(random.randint(0, 2)).squeeze()
                        #actions[idx] = torch.from_numpy(self.env.r.randint(low=0, high=2, size=(1,1))).squeeze()
                        #if self.env.env_params.additional_params['GCN']:
                            #actions[idx] = torch.from_numpy(self.env.r.randint(low=0, high=2, size=(1,1))).squeeze() 
                            #actions[list(self.env.current_graphs[self.env.graph_of_interest].parent_nid).index(self.env.current_graphs[self.env.graph_of_interest].adresses_in_graph[tl_id])] =  torch.from_numpy(self.env.r.randint(low=0, high=2, size=(1,1))).squeeze() 
                        #else:
                            #actions[idx] = torch.from_numpy(self.env.r.randint(low=0, high=2, size=(1,1))).squeeze()
                #else:
        ...
        for idx,tl_id in enumerate(self.env.Agents): 
            ...
            #if self.env.seed==0 and idx == 0:
                #print()
                #print(self.env.env_params.additional_params["yellow_duration"])
                #print("phases_defs", self.env.Agents[tl_id].phases_defs)
                ...
                if self.env.env_params.additional_params["policy"] == 'binary' and self.env.Agents[tl_id].time_since_last_action+1 >= self.env.env_params.additional_params["max_time_between_actions"]:
                    ...
                    if 'y' in self.env.Agents[tl_id].current_phase.lower():
                        if self.env.Agents[tl_id].time_since_last_action +1 < self.env.env_params.additional_params["yellow_duration"]:
                            ...
                            if self.env.env_params.additional_params["policy"] != 'binary': 
                                # IF THERE IS A YELLOW PHASE TO COMPLETE AFTER CURRENT PHASE, COMPLETE IT BEFORE CHOOSING
                        ...
                        if self.env.env_params.additional_params["policy"] != 'binary': 
                            ...
                            if action != self.env.Agents[tl_id].current_phase_idx:
                                # IF THERE IS A YELLOW PHASE TO COMPLETE AFTER CURRENT PHASE, COMPLETE IT BEFORE CHOOSING AGAIN
                                if 'y' in self.env.Agents[tl_id].phases_defs[0].state.lower():
                                    self.env.Actions[tl_id]=self.env.Agents[tl_id].current_phase_idx+1   
                                else:
        ...
        #if self.env.seed == 0:
            #for idx, tl_idx in enumerate(self.env.Agents):
                #if idx == 0: 
                    #print('action :', actions[idx], 'real action :', self.env.Actions[tl_idx])
    ...
    def setup_classes_and_networks(self):
        ...
        #self.env.Lane.position_threshold = self.env.env_params.additional_params['position_threshold']
        ...
        if self.env.seed  % self.env.n_workers == 0 :
            print("N_lanes", len(self.env.traci_connection.lane.getIDList()))
            #time.sleep(30)
            print("3.1")
        s = time.time()
        # CREATE AGENTS (TRAFFIC LIGHTS) INSTANCES AND RELEVANT CONNECTIONS OBJECTS
        #print(self.env.traci_connection.trafficlight.getIDList())
        for lane_id in self.env.traci_connection.lane.getIDList():
            if ":" not in lane_id:
                self.env.lanes.append(lane_id)
                
        if self.env.seed  % self.env.n_workers == 0 :
            print("N_tls", len(self.env.traci_connection.trafficlight.getIDList()))
            print("3.2", time.time()-s, " sec")
            #time.sleep(30)
            s = time.time()
        for tl_idx, tl_id in enumerate(self.env.traci_connection.trafficlight.getIDList()):
            self.env.Agents[tl_id]=Agent(tl_id)
            #print("programm", self.env.traci_connection.trafficlight.getCompleteRedYellowGreenDefinition(tl_id))
            #for i in self.env.traci_connection.trafficlight.getCompleteRedYellowGreenDefinition(tl_id):
                #print(i.getPhases())
                #print(i.getPhases()[0].state, help(i.getPhases()[0].state))
            #import sys
            #sys.exit()
            #print(self.env.traci_connection.trafficlight.getCompleteRedYellowGreenDefinition(tl_id)[0])
            ...
            #print("self.env.Agents[tl_id].phases_defs", type(self.env.Agents[tl_id].phases_defs), self.env.Agents[tl_id].phases_defs)
            #sys.exit()
            for idx, phase in enumerate(self.env.Agents[tl_id].phases_defs):
                phase.idx = idx
        ...
        #if self.env.seed ==0:
            #print("self.env.Lanes", self.env.Lanes)
            #print("self.env.lanes", self.env.lanes)
        #if self.env.env_params.additional_params['mode']=='train' or (self.env.env_params.additional_params['mode']=='test' and not self.env.env_params.additional_params['real_net']) : 
                    ...
                    if self.env.env_params.additional_params["grid"]:
                        if lane_id in self.env.lanes and lane_id2 in self.env.lanes and not self.env.Lanes[lane_id].inb_adj_lanes and not self.env.Lanes[lane_id2].outb_adj_lanes and set([i for i in lane_id]) == set([i for i in lane_id2]):
                            if ":" not in lane_id and ':' not in lane_id2:
                                #lane_id[lane_id.find('/')-1:lane_id.find('/')+2] != controlled_lane[controlled_lane.find('/')-1:controlled_lane.find('/')+2]:
                                is_valid_route = True
                        ...
                        if lane_id in self.env.lanes and lane_id2 in self.env.lanes and not self.env.Lanes[lane_id].inb_adj_lanes and not self.env.Lanes[lane_id2].outb_adj_lanes and lane_id.split('_')[0].split('-')[-1] != lane_id2.split('_')[0].split('-')[-1]:
                            if ":" not in lane_id and ':' not in lane_id2:
                                is_valid_route = True
                                #if self.env.seed  % self.env.n_workers == 0  :
                                    #print("origin", lane_id, "destination ", lane_id2)
    ...
    def generate_networks(self):
        ...
        for graph_name, graph in self.env.original_graphs.items():
            if graph_name != "lane_graph":
                #print('graph.subgraph(list(graph.nodes_lists["tl"])) :', graph.subgraph(list(graph.nodes_lists['tl'])).parent_nid)
                x = graph.subgraph(list(graph.nodes_lists['tl'])).parent_nid
                graph.parent_nid_ = x
                #graph.parent_nid = list(graph.subgraph(list(graph.nodes_lists['tl'])).parent_nid)
        ...
        for veh_id in arrived_ids:
            if self.env.env_params.additional_params['mode'] == 'test':
                if veh_id in self.env.trips_dict:
                    ...
                    self.env.trips_to_complete.remove(veh_id)
            #self.avg_trip_duration = self.avg_trip_duration + (self.env.trips_dict[veh_id]['trip_duration'] - self.avg_trip_duration)/self.nb_completed_trips
        ...
        #for lane_id in self.env.Lanes:
            if self.env.env_params.additional_params["mode"] == 'train':
                if self.env.env_params.additional_params["reward_type"] == 'delay':       
                    ...
                    #self.env.queue_vector[self.env.Lanes_vector_ordered.index(lane_id)] =  self.env.traci_connection.lane.getLastStepHaltingNumber(lane_id)            
            ...
            #self.env.total_queue_vector[self.env.Lanes_vector_ordered.index(lane_id)] =  self.env.Lanes[lane_id].nb_stop_veh
            #self.env.total_delay_vector[self.env.Lanes_vector_ordered.index(lane_id)] =  self.env.Lanes[lane_id].nb_stop_veh

            #self.env.queue_vector = np.asarray(self.env.queue_vector)
            #self.env.delay_vector = np.asarray(self.env.reward_vector)
        if self.env.env_params.additional_params["mode"] == 'train':
            if self.env.env_params.additional_params["reward_type"] == 'delay' :
            ...
            if self.env.step_counter > self.env.env_params.additional_params["wait_n_steps"]:                    
                ...
                #self.env.reward_vector[self.env.Lanes_vector_ordered.index(lane_id)] = -self.env.Lanes[lane_id].delay
            #else:
            #current_delay = np.asarray(self.env.delay_vector).sum()
        if self.env.env_params.additional_params['mode'] == 'test' or self.env.env_params.additional_params['save_extended_training_stats']:
            if self.env.step_counter > self.env.env_params.additional_params["wait_n_steps"]:  
                ...
                #self.env.steps_dict[self.env.step_dic_name]['delay']=np.asarray(self.env.total_delay_vector).sum()
                #current_co2 = self.env.co2
                self.env.global_co2 -= self.env.co2
                self.env.steps_co2.append(float(self.env.co2))
                #self.env.steps_dict[self.env.step_dic_name]['co2']=self.env.co2

                
                #current_queues = np.asarray(self.env.queue_vector).sum()
               ...
               # self.env.steps_dict[self.env.step_dic_name]['queue']=np.asarray(self.env.total_queue_vector).sum()
                #current_co2 = self.env.co2
    ...
    def send_results(self):
        if self.env.env_params.additional_params['mode'] == 'test':
            ...
            #self.env.tested_end.send((self.env.global_delay, self.env.global_queues, self.env.global_co2, self.env.nb_completed_trips, self.env.avg_trip_duration))
            self.env.global_delay = 0 
            self.env.global_queues = 0
            self.env.global_co2 = 0
            self.env.nb_completed_trips = 0 # EASY 
            #self.env.avg_trip_duration = 0 # DICT WITH VEHICLES (STRORE INSERTION TIME, EXIT TIME, AND THE DIFFERENCE (ADD THE DIFFERENCE TO TRIP DURATION), STORE THE DICT IN A PICKLE FILE WITH RESULTS )
        else:
            #print("YOLO")
            if self.env.greedy:
                self.env.greedy_reward_queue.put(float(self.env.global_reward))
                ...
                #self.env.reward_queue.send((float(self.env.global_reward),float(self.env.global_delay),float(self.env.global_queues), float(self.env.global_co2), float(-self.env.nb_completed_trips), float(-self.env.avg_trip_duration)))
                self.env.global_delay = 0 
                self.env.global_queues = 0
                self.env.global_co2 = 0
                self.env.nb_completed_trips = 0 # EASY 
    ...
    #def send_batch(self):
        #self.send_experience_batch()
            ...
            #self.send_experience_batch()
        #if self.env.env_params.additional_params['mode'] == 'test' :
            #self.env.tested_end.send((self.env.global_delay, self.env.global_queues, self.env.global_co2))
        #else:
            #self.send_experience_batch()
            #self.env.global_reward = 0
        #if self.env.env_params.additional_params['mode'] == 'test' or self.env.env_params.additional_params['save_extended_training_stats']:
    ...
    def init_sumo_subscriptions_and_class_variables(self):
        ...
        for lane_id in self.env.Lanes:
            ...
            #except:
                #pass
            self.env.traci_connection.lane.subscribe(lane_id, [traci.constants.LAST_STEP_VEHICLE_ID_LIST, traci.constants.LANE_LINKS]) #traci.constants.LAST_STEP_VEHICLE_HALTING_NUMBER  
