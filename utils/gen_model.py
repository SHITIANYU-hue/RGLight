...
def init_model(env):
    ...
    if not env.env_params.additional_params["GCN"]:
            ...
            #print("lane_vars",env.env_params.additional_params["lane_vars"])
        ...
        env.model2 = DNN_IQL(resnet = env.env_params.additional_params['resnet'],
                            rl_learner_type = env.env_params.additional_params['Policy_Type'],
                            policy = env.env_params.additional_params['policy'],
                            noisy = env.env_params.additional_params['noisy'],
                            double = env.env_params.additional_params['double_DQN'],
        ...
        #print("MODEL :", env.model)
        print("NUM TLS", env.model.n_tls)
        
        print( "RANDOM STATE DICT : ")
        for param_tensor in env.model.state_dict():
            print(param_tensor, "\t", env.model.state_dict()[param_tensor].size())
        for idx, i in enumerate(env.model.parameters()):
            print("param " + str(idx) + ":", i.size())
        if env.env_params.additional_params['mode'] == 'train':            
            env.model.train()
        elif env.env_params.additional_params['mode'] == 'test':
            #env.model = torch.load(env.env_params.additional_params['trained_model_path'] )
            print(env.env_params.additional_params['load_model_path'])
            
            
            
            print (" REPLACEMENT STATE DICT : ")
            print("LOADED PATH : ", env.env_params.additional_params['load_model_path'])
            ...
            for param_tensor in env.model.state_dict():
                print(param_tensor, "\t", env.model.state_dict()[param_tensor].size())   
                print('Different ? : ', (env.model.state_dict()[param_tensor] != old_state_dict[param_tensor]).any())
        ...
        print('num_nodes_types',num_nodes_types)
        ...
        #if env.env_params.additional_params["state_first_dim_only"] and env.env_params.additional_params["n_convolutional_layers"]>1:
    ...
    # POSSIBILITE DE CREER UNE CLASSE QUI INCLUE PLUSIEURS FRAMEWORK AVEC DES PARAMETRES DIFFERENTS POUR FAIRE UNE CONVOLUTION
        ...
        env.model2 = Convolutional_Message_Passing_Framework(policy, noisy, num_attention_heads, bias_before_aggregation , nonlinearity_before_aggregation, share_initial_params_between_actions, multidimensional_attention, state_first_dim_only, gaussian_mixture, n_gaussians, value_model_based = value_model_based, use_attention = use_attention, separate_actor_critic = separate_actor_critic, n_actions = n_actions, rl_learner_type = rl_learner_type, std_attention = std_attention, state_vars = state_vars, n_convolutional_layers = n_convolutional_layers, num_nodes_types = num_nodes_types, nodes_types_num_bases = nodes_types_num_bases, node_state_dim = node_state_size, node_embedding_dim = node_embedding_size, num_rels = num_rels, n_hidden_message = n_hidden_message, n_hidden_aggregation = n_hidden_aggregation, n_hidden_prediction = n_hidden_prediction, hidden_layers_size = hidden_layers_size, prediction_size = prediction_size, num_propagations = num_propagations, rel_num_bases = rel_num_bases, norm = norm, bias = True, activation = F.elu, use_message_module = use_message_module, use_aggregation_module = use_aggregation_module, dropout = dropout, resnet = resnet, dueling = dueling)

        env.model2.nodes_types = env.original_graphs[env.graph_of_interest].nodes_types
                
        print("MODEL :", env.model)
        #print("BUFFERS", [i for i in env.model.named_buffers()])
        #sys.exit()
        ...
        if value_model_based:
            ...
            print("VALUE TRANSITION MODEL :", env.model.value_transition_model[0])            
        ...
        if env.env_params.additional_params['mode'] == 'train':            
            ...
            #print("START LOAD")
            env.model.load_state_dict(torch.load(env.env_params.additional_params['load_model_path1'], map_location='cpu'))
            env.model2.load_state_dict(torch.load(env.env_params.additional_params['load_model_path2'], map_location='cpu'))
            #print("LOADED")
            env.model.eval()
...
def get_input_dims(env, lane_dim):
    ...
    for tl_id in env.Agents:
        ...
        #if env.env_params.additional_params['graph_of_interest'] == 'tl_connection_lane_graph' or env.env_params.additional_params['graph_of_interest'] == "full_graph":
