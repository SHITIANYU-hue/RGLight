...
from utils.results_process import *
import numpy as np
...
def create_exp(params, seed, real_net, real_net_address, n_workers, memory_queue,  request_end, learn_model_queue, comput_model_queue,comput_model2_queue, reward_queue, additional_reward_queue, Policy_Type, mode, greedy_reward_queue, tested_end, tested, num):
...
############################################################################################################################################
    ...
    def get_network():
        ...
        if real_net :
            ...
            if additional_env_params["grid"] == True:
                ...
                #if not (mode == 'train' or additional_env_params["specialist"] or not additional_env_params["GCN"]):
                    #additional_env_params['num_edges_random_net_train'] = additional_env_params['num_edges_random_net_test']
                
                
                # (((str(int(2000+(num*n_workers)+seed//5))) if mode == 'test' else 
    ...
    def gen_traffic():
        if additional_env_params["net_name"] == 'BIG_PARIS':
            os.system("cp sumo_nets/Real/BIG_PARIS/trips.trips.xml " + folderaddress + "/trips.trips.xml")
            os.system("cp sumo_nets/Real/BIG_PARIS/trips.rou.xml " + folderaddress + "/trips.rou.xml")
        elif additional_env_params["net_name"] == 'NY_SCP':
            os.system("cp sumo_nets/Real/NY_SCP/trips.trips.xml " + folderaddress + "/trips.trips.xml")
            os.system("cp sumo_nets/Real/NY_SCP/trips.rou.xml " + folderaddress + "/trips.rou.xml")
        #elif additional_env_params['net_name'] == 'Manhattan':
            #os.system("cp sumo_nets/Real/Manhattan/trips.trips.xml " + folderaddress + "/trips.trips.xml")
            #os.system("cp sumo_nets/Real/Manhattan/trips.rou.xml " + folderaddress + "/trips.rou.xml")            
        else:
            print('folder address :', folderaddress)
            os.system('rm '+ folderaddress + "/trips.rou.xml" + ' > /dev/null 2>&1')
            os.system('rm '+ folderaddress + "/trips.trips.xml" + ' > /dev/null 2>&1')
            os.system("python randTrips.py -n " + folderaddress +"/myNet.net.xml --demand-duration " +str(additional_env_params["demand_duration"]) + " --lane-demand-variance " +str(additional_env_params["lane_demand_variance"]) +" --min-distance "+ str(additional_env_params["min_distance"]) +" --fringe-factor " +str(additional_env_params["fringe_factor"]) + " --seed " + str(int(seed + 1 + (n_workers*num) + (200 if mode == 'test' else (step_counter+1)) + (10000 if (num == 3 and seed == 10 and additional_env_params["period"] == 2) else 0))) + " -b 0 -e "+ str(additional_env_params["nb_steps_per_exp"] * (1 if (not real_net and mode == 'test') else 1))+" --binomial 100 --period " + str(additional_env_params["period"]) + " --validate true -o " + folderaddress + "/trips.rou.xml -r " + folderaddress + "/trips.trips.xml" + ' > /dev/null 2>&1')
        ...
        for child in root:
            if int(float(child.attrib['depart'])) < 3600:
                ...
                #trips_dict[child.attrib['id']] = collections.OrderedDict()
                #for key, value in child.attrib.items():
                    #if key!='id':
                        #trips_dict[child.attrib['id']][key] = value 
        #print(trips_dict)
        return trips_dict
            # (to keep same traffic for every synchronous tests)
            #os.system("python randTrips.py -n " + folderaddress +"/myNet.net.xml --seed " + (str(seed + (n_workers*num)) if (mode == 'train') else str(100)) + " -b 0 -e "+ str(additional_env_params["exp_duration"])+" --binomial 100 --period " + str(additional_env_params["period"]) + " --validate true -o " + folderaddress + "/trips.rou.xml -r " + folderaddress + "/trips.trips.xml")
    ...
    def gen_folders():
        ...
        if mode == 'train':
            ...
            additional_env_params['load_model_path1'] +=  '/' + str(num) + '/' + "params_checkpoint.pt"           
            additional_env_params['load_model_path2'] +=  '/' + str(num) + '/' + "params_checkpoint.pt"           
            additional_env_params['rendering_path'] +=  '/' + str(tested) 
            #os.system("rm -r " + params["rendering_path"]) 
...
############################################################################################################################################
    ...
    #print("num_spe",params['num_specialist'] )
    if seed < int(params['num_specialist']):
        #print(seed, "specialist")
    ...
    #print("n_avg_text", params['n_avg_test'])
    ...
    try:    
        ...
        while True :
            ...
            step_counter += exp.run(trips_dict, epoch, seed, num, n_workers, memory_queue,  request_end, learn_model_queue, comput_model_queue, comput_model2_queue,reward_queue, additional_reward_queue, Policy_Type, mode, greedy_reward_queue, tested_end, tested)
...
def run_experiment(params, num = 0):
    ...
    #assert (real_net_address is None) if real_net == Fal, 'if using a real network, please provide the path as "real_net_address"'
    from pathos.helpers import mp as multiprocess
    import collections
    import random
    import torch 
    #import multiprocessing as mp
    #from multiprocessing import Pool, Timehttps://pypi.org/project/pathos/outError
    ...
    comput_model2_queue = manager.Queue(1)
    ...
    if Policy_Type == "Q_Learning":
        learn_process =  multiprocess.Process(target=train, args=(memory_other_ends,
                                                                  ...
                                                                  num))
        comput_process =  multiprocess.Process(target=comput, args=(request_comput_ends, comput_model_queue,comput_model2_queue))
        learn_process.start()
    else:
        comput_process =  multiprocess.Process(target=comput, args=(request_comput_ends, 
                                                                    comput_model_queue,
                                                                    comput_model2_queue,
                                                                    baseline_reward_queue if mode == 'test' else None,
                                                                    greedy_reward_queue if (greedy_training or mode == 'test') else None,
                                                                    ...
                                                                    tested if mode == 'test' else None,
                                                                    reward_other_ends))
        results_process =  multiprocess.Process(target=results, args=(n_workers,
                                                                      params,
                                                                     baseline_reward_queue if mode == 'test' else None,
                                                                     greedy_reward_queue if (greedy_training or mode == 'test') else None,
                                                                     tested_learner_ends if mode == 'test' else None,
...
if __name__ == '__main__':
    ...
    for file in params['config_dirs']:
        ...
        # IF EVALUATING
        if params['mode'] == 'test':
            # ITERATE OVER ALL THE 'KINDS' OF POLICY
            for tested_policy in params['tests']:  
                params['tested']= []
                # FOR A GIVEN RUN/EXPERIMENT, WE EVALUATE EVERY 'KIND' OF POLICY USING 'N_TESTS' PARALLEL ENVIRONMENTS (e.g. 30)
